...
function DP9(A) {
  let G = {
    ...
    D = A.COMMENT('<!--', '-->', { relevance: 0 }),
    Z = {
      className: 'meta',
      begin: '\\[noprocess\\]',
      starts: { end: '\\[/noprocess\\]', returnEnd: !0, contains: [D] },
    },
    Y = { className: 'meta', begin: '\\[/noprocess|<\\?(lasso(script)?|=)' },
    W = { className: 'symbol', begin: "'[a-zA-Z_][\\w.]*'" },
    F = [
      A.C_LINE_COMMENT_MODE,
      A.C_BLOCK_COMMENT_MODE,
      A.inherit(A.C_NUMBER_MODE, { begin: A.C_NUMBER_RE + '|(-?infinity|NaN)\\b' }),
      ...
      { begin: /(->|\.)\s*/, relevance: 0, contains: [W] },
      {
        className: 'class',
        beginKeywords: 'define',
        returnEnd: !0,
        end: '\\(|=>',
        contains: [A.inherit(A.TITLE_MODE, { begin: '[a-zA-Z_][\\w.]*(=(?!>))?|[-+*/%](?!>)' })],
      },
    ];
  return {
    name: 'Lasso',
    ...
    contains: [
      {
        ...
        begin: '\\]|\\?>',
        relevance: 0,
        starts: { end: '\\[|<\\?(lasso(script)?|=)', returnEnd: !0, relevance: 0, contains: [D] },
      },
      Z,
      Y,
      {
