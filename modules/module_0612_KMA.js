...
function eS9(A) {
  ...
  return {
    name: 'Stan',
    aliases: ['stanfuncs'],
    keywords: { $pattern: A.IDENT_RE, title: B, keyword: Q.concat(G).concat(I), built_in: D },
    contains: [
      A.C_LINE_COMMENT_MODE,
      A.COMMENT(/#/, /$/, { relevance: 0, keywords: { 'meta-keyword': 'include' } }),
      A.COMMENT(/\/\*/, /\*\//, {
        relevance: 0,
      ...
      { begin: '~\\s*(' + A.IDENT_RE + ')\\s*\\(', keywords: Z },
      {
        className: 'number',
        variants: [
          { begin: /\b\d+(?:\.\d*)?(?:[eE][+-]?\d+)?/ },
