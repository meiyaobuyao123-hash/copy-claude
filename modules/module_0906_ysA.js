// Module: ysA
// Params: Up5,jsA

var { defineProperty: _B1, getOwnPropertyDescriptor: TI4, getOwnPropertyNames: PI4 } = Object,
  SI4 = Object.prototype.hasOwnProperty,
  wM = (A, B) => _B1(A, 'name', { value: B, configurable: !0 }),
  _I4 = (A, B) => {
    for (var Q in B) _B1(A, Q, { get: B[Q], enumerable: !0 });
  },
  jI4 = (A, B, Q, I) => {
    if ((B && typeof B === 'object') || typeof B === 'function') {
      for (let G of PI4(B))
        if (!SI4.call(A, G) && G !== Q)
          _B1(A, G, { get: () => B[G], enumerable: !(I = TI4(B, G)) || I.enumerable });
    }
    return A;
  },
  yI4 = (A) => jI4(_B1({}, '__esModule', { value: !0 }), A),
  TsA = {};
_I4(TsA, {
  Field: () => fI4,
  Fields: () => vI4,
  HttpRequest: () => bI4,
  HttpResponse: () => gI4,
  IHttpRequest: () => PsA.HttpRequest,
  getHttpHandlerExtensionConfiguration: () => kI4,
  isValidHostname: () => _sA,
  resolveHttpHandlerRuntimeConfig: () => xI4,
});
jsA.exports = yI4(TsA);
var kI4 = wM((A) => {
    return {
      setHttpHandler(B) {
        A.httpHandler = B;
      },
      httpHandler() {
        return A.httpHandler;
      },
      updateHttpClientConfig(B, Q) {
        A.httpHandler?.updateHttpClientConfig(B, Q);
      },
      httpHandlerConfigs() {
        return A.httpHandler.httpHandlerConfigs();
      },
    };
  }, 'getHttpHandlerExtensionConfiguration'),
  xI4 = wM((A) => {
    return { httpHandler: A.httpHandler() };
  }, 'resolveHttpHandlerRuntimeConfig'),
  PsA = jP1(),
  fI4 = class {
    static {
      wM(this, 'Field');
    }
    constructor({ name: A, kind: B = PsA.FieldPosition.HEADER, values: Q = [] }) {
      ((this.name = A), (this.kind = B), (this.values = Q));
    }
    add(A) {
      this.values.push(A);
    }
    set(A) {
      this.values = A;
    }
    remove(A) {
      this.values = this.values.filter((B) => B !== A);
    }
    toString() {
      return this.values.map((A) => (A.includes(',') || A.includes(' ') ? `"${A}"` : A)).join(', ');
    }
    get() {
      return this.values;
    }
  },
  vI4 = class {
    constructor({ fields: A = [], encoding: B = 'utf-8' }) {
      ((this.entries = {}), A.forEach(this.setField.bind(this)), (this.encoding = B));
    }
    static {
      wM(this, 'Fields');
    }
    setField(A) {
      this.entries[A.name.toLowerCase()] = A;
    }
    getField(A) {
      return this.entries[A.toLowerCase()];
    }
    removeField(A) {
      delete this.entries[A.toLowerCase()];
    }
    getByType(A) {
      return Object.values(this.entries).filter((B) => B.kind === A);
    }
  },
  bI4 = class A {
    static {
      wM(this, 'HttpRequest');
    }
    constructor(B) {
      ((this.method = B.method || 'GET'),
        (this.hostname = B.hostname || 'localhost'),
        (this.port = B.port),
        (this.query = B.query || {}),
        (this.headers = B.headers || {}),
        (this.body = B.body),
        (this.protocol = B.protocol
          ? B.protocol.slice(-1) !== ':'
            ? `${B.protocol}:`
            : B.protocol
          : 'https:'),
        (this.path = B.path ? (B.path.charAt(0) !== '/' ? `/${B.path}` : B.path) : '/'),
        (this.username = B.username),
        (this.password = B.password),
        (this.fragment = B.fragment));
    }
    static clone(B) {
      let Q = new A({ ...B, headers: { ...B.headers } });
      if (Q.query) Q.query = SsA(Q.query);
      return Q;
    }
    static isInstance(B) {
      if (!B) return !1;
      let Q = B;
      return (
        'method' in Q &&
        'protocol' in Q &&
        'hostname' in Q &&
        'path' in Q &&
        typeof Q.query === 'object' &&
        typeof Q.headers === 'object'
      );
    }
    clone() {
      return A.clone(this);
    }
  };
function SsA(A) {
  return Object.keys(A).reduce((B, Q) => {
    let I = A[Q];
    return { ...B, [Q]: Array.isArray(I) ? [...I] : I };
  }, {});
}
wM(SsA, 'cloneQuery');
var gI4 = class {
  static {
    wM(this, 'HttpResponse');
  }
  constructor(A) {
    ((this.statusCode = A.statusCode),
      (this.reason = A.reason),
      (this.headers = A.headers || {}),
      (this.body = A.body));
  }
  static isInstance(A) {
    if (!A) return !1;
    let B = A;
    return typeof B.statusCode === 'number' && typeof B.headers === 'object';
  }
};
function _sA(A) {
  return /^[a-z0-9][a-z0-9\.\-]*[a-z0-9]$/.test(A);
}
wM(_sA, 'isValidHostname');
